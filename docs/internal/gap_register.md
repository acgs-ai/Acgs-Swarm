...
| GAP-V01-ADVERSARIAL-BENCHMARK | Focused adversarial benchmark | artifact-pack scaffold plus result gate defined; external study open | `src/constitutional_swarm/governance_fixtures.py`; `src/constitutional_swarm/forensic_benchmark.py`; `scripts/run_governance_benchmark.py --protocol-manifest`; `scripts/run_governance_benchmark.py --generate-incident-pack`; `scripts/run_governance_benchmark.py --validate-result-bundle`; `tests/test_governance_receipts.py` | Runnable offline DevOps workloads cover collusion, slow-burn harm, and provenance forgery as local conformance fixtures, and the public-study scaffold generates 50 matched artifacts with hidden answer keys separated. Result validation rejects toy/non-external success claims. Public v0.1 success still requires real blind-review collection over 50 to 200 adversarial incidents and scored significant results. |
| GAP-V01-ADVERSARIAL-BENCHMARK | Focused adversarial benchmark | artifact-pack scaffold plus result gate defined; synthetic local end-to-end evidence now verified; external study still open | `src/constitutional_swarm/governance_fixtures.py`; `src/constitutional_swarm/forensic_benchmark.py`; `scripts/run_governance_benchmark.py --protocol-manifest`; `scripts/run_governance_benchmark.py --generate-incident-pack`; `scripts/run_governance_benchmark.py --validate-result-bundle`; `tests/test_governance_receipts.py` | Runnable offline DevOps workloads cover collusion, slow-burn harm, and provenance forgery as local conformance fixtures, and the public-study scaffold generates 50 matched artifacts with hidden answer keys separated. Fresh local smoke on 2026-05-16 validated incident pack generation, reviewer packet generation, result-bundle construction, scorecard validation, reviewer-cohort validation, replication metadata, and attestation on synthetic data. Result validation still rejects toy/non-external success claims. Public v0.1 success still requires real blind-review collection over 50 to 200 adversarial incidents and scored significant results. |
| GAP-V01-STRONG-BASELINE | Non-strawman baseline | protocol defined; local scorecard and baseline comparison evidence now verified; external study still open | `src/constitutional_swarm/forensic_benchmark.py`; `scripts/run_governance_benchmark.py --protocol-manifest`; `tests/test_governance_receipts.py` | The study protocol requires matched artifact conditions for ungoverned raw logs, centralized structured logs, and ACGS receipts/audit artifacts, and scores ACGS against the strongest non-ACGS baseline. Fresh local validation shows the scorecard and result bundle can be built and validated, but closure still requires collected reviewer results and public evidence, not only the local fixture comparator. |
| GAP-V01-REPRODUCTION-BUDGET | External adoption budget | local replication commands and result gate defined; synthetic attestation path verified; external replication still open | `src/constitutional_swarm/forensic_benchmark.py`; `scripts/run_governance_benchmark.py --protocol-manifest`; `scripts/run_governance_benchmark.py --generate-incident-pack`; `scripts/run_governance_benchmark.py --score-reviewer-answers --answer-key-json`; `scripts/run_governance_benchmark.py --build-result-bundle --answer-key-json`; `scripts/run_governance_benchmark.py --validate-result-bundle`; `tests/test_governance_receipts.py` | Benchmark reports command, model/backend, wall-clock, tokens, estimated cost, and runs the default conformance suite offline with zero token and dollar cost. Fresh local validation shows the result bundle, scorecard, reviewer cohort manifest, replication metadata, and attestation can all validate on synthetic data, but closure still requires a non-ACGS rerun bundle with generated artifact sets, blind-review answers that omit ground truth, significant scorecard, and completed external replication metadata from outside this checkout. |
