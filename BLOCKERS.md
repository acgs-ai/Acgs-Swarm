...
| B3 | **Open** | Medium | No static type checker (mypy/pyright) is configured, though the code is type-annotated. `make typecheck` runs ruff as a stand-in. | maintainers | Add mypy or pyright config + a CI job; point `make typecheck` at it. |
