...
## Purpose
Pytest suite covering every module in `src/constitutional_swarm`. Do not hardcode a live test total. Expected xfails include Birkhoff uniformity collapse (research control). Tests are organized one-to-one with source modules plus a few cross-cutting integration suites.
